    ...
    public String parseFormula(StringTokenizer tokens, int depth) 
        ...
        if (tokens.hasMoreTokens()) {
            ...
            if (tok.equals("SUM")) {
            	...
            	if (!close.equals(")")) return null; 
            	
            	if (tokens.hasMoreTokens()) return null; 
            	
            	if (start.substring(1).equals(end.substring(1))) { // same row
            		int first = start.charAt(0); 
            		int last = end.charAt(0);
            		int bigger = Math.max(first, last); 
            		int smaller = Math.min(first, last);
            		
            		String sum = "0"; 
            		for (int i = smaller; i <= bigger; i++) {
            			String addend = (char) i + start.substring(1);
            			addend = evaluateToken(addend, depth);
            			if (addend == null) return null; 
            			sum = add(sum, addend); 
            		}
            		return sum;
            	} else if (start.charAt(0) == end.charAt(0)) { // same column
	            	int first = Integer.parseInt(start.substring(1)); 
	        		int last = Integer.parseInt(end.substring(1));
	        		int bigger = Math.max(first, last); 
	        		int smaller = Math.min(first, last);
	        		
	        		String sum = "0"; 
	        		for (int i = smaller; i <= bigger; i++) {
	        			String addend = evaluateToken(start.charAt(0) + i + "", depth);
	        			if (addend == null) return null; 
	        			sum = add(sum, addend); 
	        		}
	        		return sum;
